...
from database.sqlalchemy_models import Base, Feed, Episode, Digest
...
def migrate_feeds(sqlite_conn: sqlite3.Connection, session: Session) -> int:
    ...
    for r in rows:
        d = row_to_dict(r)
        # Don't include ID to avoid conflicts - let Postgres auto-generate
...
def migrate_episodes(sqlite_conn: sqlite3.Connection, session: Session) -> int:
    ...
    for r in rows:
        ...
        # Don't include ID in the insert to avoid conflicts - let Postgres auto-generate
...
def migrate_digests(sqlite_conn: sqlite3.Connection, session: Session) -> int:
    ...
    for r in rows:
        ...
        ).on_conflict_do_nothing(index_elements=[Digest.topic, Digest.digest_date])
        session.execute(stmt)
        count += 1
    return count
...
def main() -> None:
    ...
    sqlite_path = project_root / "data" / "database" / "_digest.db"
